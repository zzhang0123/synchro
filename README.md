...
A plain run skips the 1635 tests marked `slow` (1573 of them in the
automatic-differentiation transform matrix, the others mostly the
full-resolution benchmark); run them with `python -m pytest tests -m slow`
or `SYNCMOMENTS_RUN_SLOW=1`.
...
Equinox 0.13.8, NumPy 2.5.3, SciPy 1.18.1 without mpmath. The tests do not
import mpmath: their high-precision references are literals generated by
`scripts/reference_constants.py`. The benchmark tests use a verbatim,
...
through `vmap` and nested `vmap` over batched `K` or `ds`; the value's 5x5
exponential is `jax.scipy.linalg.expm`, whose `floor` count applies Pade 13
up to `2 theta_13` (relative error 9.6e-12 for a Faraday-dominated slab
just below `|K ds| = 10.74`, growing with the number of squarings, and
1.2e-10 to 4.4e-10 for realistic rotation-dominated slabs of a
`moment_driven_slab_cgs` spectrum). Second derivatives in
