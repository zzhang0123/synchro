...
def test_moment_driven_slab_thin_absorbed_and_rotated():
    ...
    assert rot_err < 1e-5  # finite internal rotation and sinc depolarisation
...
# --- finite Bell/Taylor contractions ---------------------------------------------


def test_gaussian_bell_sum_converges_for_exponential():
    approx, exact, raw2 = validate_cumulants.test_gaussian_exponential_finite_taylor()
    assert (
        abs(approx - exact) / exact < 1e-8
    )  # Gaussian statistics, finite Taylor degree N=14
...
def test_absorbed_polarisation_fraction_thin_and_thick_limits():
    ...
    # thick boundary; Pi -> (S_plus - S_minus)/(S_plus + S_minus)
    pi_thick = float(absorbed_polarisation_fraction(*args, 1e12, ig))
    assert abs(pi_thick - (S_plus - S_minus) / (S_plus + S_minus)) < 1e-12
...
def test_absorbed_intensity_thin_limit_is_j_times_L():
    ...
    for L in (1e-20, 1e-14, 1e-8):
        I = float(absorbed_intensity_from_moments(100.0, 30.0, 1.0, M0, M1, M2, L, ig))
        assert I > 0.0
...
def test_moment_slab_polarised_absorption_matches_kirchhoff():
    ...
    for L in (1e-2, 1e2, 1e4, 1e6, 1e10):
        S = moment_driven_slab(*args, ig, L=L, n_slabs=256, polarised_absorption=True)
...
def test_moment_slab_rejects_imposed_polarisation_with_kirchhoff_absorption():
    ...
    with pytest.raises(ValueError):
        moment_driven_slab(
            100.0, 30.0, 1.0, M0, M1, M2, ig, L=1.0, Q0=0.2, polarised_absorption=True
        )
...
def _gauss_hermite_ensemble(
    n, gamma0, alpha0, theta0, sig_g=0.0, sig_a=0.0, n_nodes=21
):
    """Numerical Gaussian average of normalized I_n by Gauss-Hermite quadrature.

    This is a finite quadrature check of the normalized response. Nodes stay
    above gamma=1 for the quoted widths; the full Gaussian still has unphysical
    ...
    vals = np.array(
        [
            float(
                stokes_harmonic(n, gamma0 + sig_g * xi, alpha0 + sig_a * xi, theta0)[0]
            )
            for xi in x
        ]
    )
...
def test_convergence_is_fourth_order_in_the_width():
    ...
    for f in fracs:
        sig = f * g0
        ap = np.asarray(exp(jnp.zeros(3), jnp.diag(jnp.array([sig**2, 0.0, 0.0]))))[
            :, 0
        ]
        ex = np.array([_gauss_hermite_ensemble(n, g0, a0, t0, sig_g=sig) for n in ns])
...
def test_pitch_angle_is_the_binding_direction():
    ...
    sig_g = 0.06 * g0
    ap_g = np.asarray(exp(jnp.zeros(3), jnp.diag(jnp.array([sig_g**2, 0.0, 0.0]))))[
        :, 0
    ]
    ex_g = np.array([_gauss_hermite_ensemble(n, g0, a0, t0, sig_g=sig_g) for n in ns])
    err_g = np.abs(ap_g - ex_g) / np.abs(ex_g)

    sig_a = 0.1
    ap_a = np.asarray(exp(jnp.zeros(3), jnp.diag(jnp.array([0.0, sig_a**2, 0.0]))))[
        :, 0
    ]
    ex_a = np.array([_gauss_hermite_ensemble(n, g0, a0, t0, sig_a=sig_a) for n in ns])
...
def test_mildly_relativistic_claims_of_section_5_4():
    ...
    for g, nmax in ((2.0, 200), (5.0, 400)):
        I = np.zeros(nmax - 1)
        V = np.zeros(nmax - 1)
        # Same finite grid and sums, compiled once over the harmonic vector.
        # Streaming the angles avoids a large (angles,harmonics,nodes) array.
        ns = jnp.arange(1, nmax)
        kernel = jax.jit(lambda a: jnp.stack(stokes_harmonic(ns, g, a, th)))
        for mu in mus:
            i, _, v = np.asarray(kernel(float(np.arccos(mu))))
            I += i
            V += v
        tot = I.sum()
        c = np.cumsum(I)
        out[g] = (V.sum() / tot, I[0] / tot, int(np.searchsorted(c, 0.9 * tot)) + 1)
