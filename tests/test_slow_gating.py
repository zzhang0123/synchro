...
A ``slow`` test runs with ``-m slow`` or ``SYNCHRO_RUN_SLOW=1`` and is
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("", {"SYNCHRO_RUN_SLOW": "1"}, True),
        ("", {"SYNCHRO_RUN_SLOW": "0"}, False),
        ("", {"SYNCHRO_RUN_SLOW": ""}, False),
        ("not slow", {"SYNCHRO_RUN_SLOW": "1"}, True),
    ],
)
...
def run(tmp_path, *args, env_value=None):
    ...
    env = {k: v for k, v in os.environ.items() if k != "SYNCHRO_RUN_SLOW"}
    if env_value is not None:
        env["SYNCHRO_RUN_SLOW"] = env_value
    out = subprocess.run(
