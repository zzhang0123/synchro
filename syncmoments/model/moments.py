"""Population statistics of the finite joint response (``syncmoments.model.moments``).
...
in ``syncmoments.rm._screen_samples``.
...
class JointMoments(eqx.Module):
    ...
    def to_dict(self) -> dict:
        ...
        ``index.max_orders`` is ``null`` for the uncapped cutoff or the list
        ``[N_gamma, N_B, N_depth]`` of a capped truncation, so the layout of
        ``vector`` is identifiable from the dict alone.
        """
