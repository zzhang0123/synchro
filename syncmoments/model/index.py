"""Index layout of the finite joint response (``syncmoments.model.index``).
...
``[extension]`` per-variable caps ``max_orders=(N_gamma, N_B, N_depth)``
intersect the cutoffs with ``r <= N_gamma, s <= N_B, b <= N_depth`` (a
lower set; its remainder margin is ``lower_set_margin``).
...
live in ``syncmoments.model.moments``; nothing in this module is a physical
...
from ._remainder_route import lower_set_margin
...
class Truncation(eqx.Module):
    """Static truncation orders ``(L_mu, L_eta, N, depth_degree, max_orders)``.
    ...
    ``max_orders=(N_gamma, N_B, N_depth)`` (keyword addition ``[extension]``;
    each a static int or ``None``) also requires ``r <= N_gamma``,
    ``s <= N_B``, ``b <= N_depth``. The retained ``(r, s, b)`` set is then a
    lower set whose Taylor remainder is the margin sum of
    :func:`lower_set_margin` (``bounds.RemainderInputs`` uses it when
    :meth:`is_capped`). Caps that remove no row are dropped, so a
    non-binding ``max_orders`` equals the default; in the ``app: depth
    moments`` layout ``N_depth < depth_degree`` raises ``ValueError`` (set
    ``depth_degree`` instead). All fields are static and hashable; the
    module has no array leaves. Dimensionless. Assumes nothing about the
    population; the orders are declared cutoffs, and nothing here certifies
    that they are sufficient (the remainder terms of ``eq: channel error
    budget`` do that).
    """
    ...
    max_orders: tuple[int | None, int | None, int | None] | None = eqx.field(
        static=True, default=None
    )

    def __init__(self, L_mu, L_eta, N, depth_degree=None, *, max_orders=None):
        ...
        self.max_orders = _binding_caps(max_orders, self.N, self.depth_degree)

    def caps(self) -> tuple[int, int, int]:
        """Effective per-variable limits ``(r_max, s_max, b_max)``."""
        caps = self.max_orders or (None, None, None)
        depth = self.N if self.depth_degree is None else self.depth_degree
        limits = (self.N, self.N, depth)
        return tuple(l if c is None else c for l, c in zip(limits, caps))

    def max_b(self) -> int:
        """Largest retained depth power: ``N``, ``depth_degree`` or ``N_depth``."""
        return self.caps()[2]

    def is_capped(self) -> bool:
        """True when ``max_orders`` removes rows (the lower-set remainder applies)."""
        return self.max_orders is not None

    def margin_rows(self) -> tuple[tuple[int, int, int], ...]:
        """Margin ``S(Lambda)`` of the retained Taylor set, as ``(r, s, b)``.

        Main-text layout: ``S`` of the ``(r, s, b)`` set of the ``P`` rows (the
        ``I, V`` set is its ``b = 0`` section, whose margin is the ``b = 0``
        part of this one). ``app: depth moments`` layout: ``S`` of the
        ``(r, s)`` set with ``b = 0`` (the depth is bounded by the depth tail).
        """
        if self.depth_degree is None:
            return lower_set_margin(_rsb_rows(self))
        return tuple((r, s, 0) for r, s in lower_set_margin(_rs_rows(self)))


def _binding_caps(max_orders, N, depth_degree):
    """Validated caps with non-binding entries replaced by ``None``."""
    if max_orders is None:
        return None
    if not isinstance(max_orders, tuple) or len(max_orders) != 3:
        raise ValueError("max_orders must be a tuple (N_gamma, N_B, N_depth)")
    names = ("N_gamma", "N_B", "N_depth")
    caps = [None if c is None else _static_int(c, n) for c, n in zip(max_orders, names)]
    limits = (N, N, N if depth_degree is None else depth_degree)
    if depth_degree is not None and caps[2] is not None and caps[2] < depth_degree:
        raise ValueError(
            f"N_depth={caps[2]} < depth_degree={depth_degree}: in the app: depth "
            "moments layout set the depth order with depth_degree"
        )
    caps = tuple(None if c is None or c >= l else c for c, l in zip(caps, limits))
    return None if caps == (None, None, None) else caps
...
def _rs_rows(truncation):
    N = truncation.N
    r_max, s_max, _ = truncation.caps()
    return [(r, s) for r in range(r_max + 1) for s in range(min(N - r, s_max) + 1)]


def _rsb_rows(truncation):
    N, b_max = truncation.N, truncation.max_b()
    if truncation.depth_degree is None:
        return [
            (r, s, b)
            for r, s in _rs_rows(truncation)
            for b in range(min(N - r - s, b_max) + 1)
        ]
    return [(r, s, b) for r, s in _rs_rows(truncation) for b in range(b_max + 1)]
...
class MomentIndex(eqx.Module):
    ...
    * ``h0_ext``: real ``M0`` rows with ``b >= 1`` over the ``h2`` pair set
    ...
    ``n_+ = [(L_mu+1)(L_eta+1) + d]/2`` in the main-text layout. With
    ``max_orders`` every ``(r, s, b)`` cutoff above also obeys the caps.
    ...
    def __init__(self, truncation, *, parity=True, components=("I", "Q", "V")):
        ...
        L_mu, L_eta = truncation.L_mu, truncation.L_eta
        ...
        rs = _rs_rows(truncation)
        self.h0 = tuple((l, k, r, s, 0) for l, k in h0_pairs for r, s in rs)
...
def main_text_sizes(truncation: Truncation) -> tuple[int, int, int]:
    ...
    certificate). ``ValueError`` for a capped truncation (no closed form).
    """
    if truncation.is_capped():
        raise ValueError("main_text_sizes covers uncapped truncations only")
