...
Private helper of ``syncmoments.model.moments`` (split to keep files short); the
...
in ``syncmoments.rm._screen_samples``. Shape errors raise ``ValueError`` at trace
...
from ..rm import _screen_samples, _validated_samples
...
@eqx.filter_jit  # ``names`` (strings) are static
def _product_measure(values, weights, names):
    """Grids and product masses of 1D marginals in one compiled call.

    Each marginal is checked and normalised by ``_validated_samples``, the
    helper behind ``_screen_samples`` (same float64 arithmetic), so the masses
    equal per-marginal normalisation; one compilation replaces one per marginal.
    """
    checked = [_validated_samples(*entry) for entry in zip(values, weights, names)]
    grids = jnp.meshgrid(*(v for v, _ in checked), indexing="ij")
    mass = jnp.ones(())
    for g in jnp.meshgrid(*(w for _, w in checked), indexing="ij"):
        mass = mass * g
    return [g.ravel() for g in grids], mass.ravel()


@eqx.filter_jit
def _checked_samples(arrays, weights):
    """Value checks of ``PopulationSamples`` in one compiled call per shape.

    Returns the checked arrays and the normalised weights (``None``: uniform),
    whose check is that of ``_screen_samples``."""
    ...
    arrays["B"] = eqx.error_if(arrays["B"], jnp.any(arrays["B"] < 0), "B must be >= 0")
    for name in ("mu", "eta"):
    ...
    return arrays, _validated_samples(arrays["gamma"], weights, "gamma")[1]
...
class PopulationSamples(eqx.Module):
    ...
    def __init__(self, gamma, B, mu, eta, phi, depth, weights=None):
        ...
        # The normalised weights are only checked (weights are stored raw);
        # inside an outer JIT the unused check is dropped and runs on use.
        arrays, _ = _checked_samples(arrays, weights)
    ...
    def product(cls, **marginals) -> "PopulationSamples":
        ...
        for name in VARIABLES:
            ...
            if v.size == 0:
                raise ValueError(f"{name} samples must be a nonempty 1D array")
            values.append(v)
            weights.append(w)
        grids, mass = _product_measure(values, weights, VARIABLES)  # overflow-safe
        return cls(*grids, weights=mass)
