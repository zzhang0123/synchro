...
``syncmoments.faraday.joint_faraday_average``), ``extra eq: channel kernel``
against the fixed-harmonic quadratic average of ``syncmoments.expansion``
(:func:`fixed_harmonic_comparison`; two different observables, not an
equality), ``eq: independent gaussian screen`` against
``syncmoments.rm.burn_depolarisation`` (:func:`burn_screen_check`).
...
Units follow ``syncmoments.model.predict``: channel Stokes are the amplitude
...
def fixed_harmonic_comparison(
    ...
    ``expansion`` is a ``syncmoments.expansion.CumulantExpansion`` built at
    ...
    direct reference is ``syncmoments.stokes.stokes_harmonic`` at every sample.
