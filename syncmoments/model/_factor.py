...
Private helper of :mod:`syncmoments.model.assumptions`; nothing here is public API.
...
(``gamma``, Gauss, cosines, radians, rad/m^2). Symmetry closures
(``SYMMETRY_KINDS``) have the empty group ``()``: they constrain the joint
measure, close no marginal, and only drop rows (``pitch_symmetric``: odd
``l``; ``field_reversal_symmetric``: odd ``l`` or odd ``k`` when ``mu`` and
``eta`` lie in different groups, else odd ``l + k``; see :func:`drops_row`);
a ``fixed_table`` under a symmetry takes the full table and ignores the
entries the symmetry drops (:func:`symmetrised_tables`).
Nothing here certifies that a closure describes the population; it only
evaluates the closure's moments.
"""
...
SYMMETRY_KINDS = ("pitch_symmetric", "field_reversal_symmetric")
REFERENCE_FREE = ("uniform_mu", "fixed_table") + SYMMETRY_KINDS


FLIPPED = {"pitch_symmetric": ("mu",), "field_reversal_symmetric": ("mu", "eta")}


def drops_row(kind, row, groups=(VARS,)):
    """True when a closure of ``kind`` sets the 6-tuple ``row`` to zero exactly.

    ``uniform_mu``: ``l > 0``. A symmetry ``g`` acts variable by variable:
    ``pitch_symmetric`` flips ``mu``, ``field_reversal_symmetric`` flips
    ``mu`` and ``eta`` and shifts ``phi`` by ``pi`` (``e^{i h pi} = 1`` for
    ``h in {0, 2}``) at fixed ``gamma, B, depth``. Under the product measure
    ``p = prod_G p_G`` of ``groups``, ``g`` maps each group to itself, so the
    marginal of ``p o g`` on ``G`` is ``p_G o g_G``: the joint invariance
    ``p o g = p`` holds iff every marginal is invariant. By ``P_l(-x) =
    (-1)^l P_l(x)`` a group factor whose flipped exponents have odd sum is
    then zero, and so is the row. With one group this is the joint rule
    (odd ``l``; odd ``l + k``); with ``mu`` and ``eta`` in different groups
    field reversal drops odd ``l`` and odd ``k`` rows, so no free table keeps
    ``<P_odd(mu) ...>`` or ``<P_odd(eta) ...>``.
    """
    if kind == "uniform_mu":
        return row[1] > 0
    flipped = FLIPPED.get(kind, ())
    return any(
        sum(row[ROW_POSITION[v]] for v in group if v in flipped) % 2 == 1
        for group in groups
    )


def closure_key(closure):
    """Identity of a closure slot: its group, and its kind for a symmetry."""
    kind = closure.kind if closure.kind in SYMMETRY_KINDS else None
    return (tuple(closure.group), kind)
...
def build_tables(index, groups, closures):
    ...
    tables are formed; also the rows with an odd group factor under the
    symmetry closures, see :func:`drops_row`) and whether every kept
    ``h0_ext`` row is covered.
    """
    h0, ext, h2 = retained_rows(index)
    all_rows = h0 + ext + h2
    n0, n_ext = len(h0), len(ext)
    keep = [
        not any(drops_row(c.kind, row, groups) for c in closures) for row in all_rows
    ]
...
def symmetrised_tables(index, groups, closures, spec):
    """Per closure ``(n_full, kept, note)`` for a ``fixed_table`` a symmetry trims.

    A closure combined with a symmetry closure represents its symmetrised
    marginal. A ``fixed_table`` takes its table over the group's ``n_full``
    entries in the same partition without the symmetry closures (the shape
    it has on the map that ``assume`` combines with the symmetry); only the
    positions ``kept`` (those of ``spec``) enter. The others are ignored:
    entries the symmetry sets to zero (odd flipped exponents on the group,
    :func:`drops_row`) and any entry no kept row uses; ``note`` names them
    for errors and the record. ``None`` for other closures and when no entry
    is dropped.
    """
    fixed = [c.kind == "fixed_table" for c in closures]
    plain = tuple(c for c in closures if c.kind not in SYMMETRY_KINDS)
    if len(plain) == len(closures) or not any(fixed):
        return (None,) * len(closures)
    full, out = build_tables(index, groups, plain)[0], []
    for closure, is_fixed in zip(closures, fixed):
        g = groups.index(tuple(closure.group)) if is_fixed else None
        if g is None or full[g] == spec[g]:
            out.append(None)
            continue
        group, entries = tuple(closure.group), full[g]
        drop = ", ".join(entry_label(group, p) for p in entries if p not in spec[g])
        note = f" (full table, symmetrised: dropped {drop})"
        out.append((len(entries), tuple(entries.index(p) for p in spec[g]), note))
    return tuple(out)
...
def validate_closure(closure, groups, index):
    ...
    if closure.kind in SYMMETRY_KINDS:
        if group != () or closure.hyper:
            raise ValueError(
                f"{closure.kind} has the empty group () and no hyper-parameters"
            )
        return
