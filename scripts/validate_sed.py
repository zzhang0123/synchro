...
4. absolute emissivity (CGS) == (sqrt3 e^3 C B/(8 pi m_e c^2)) * j_num(nu/nu_B),
   with the normalised angular probability measure.
"""
...
from synchro.sed import (
    ...
    power_law_emissivity_abs,
    E_ESU,
    M_E,
    C_CGS,
    los_intensity,
    spectral_index_cumulants,
)
...
def j_num_double(p, nu, ng=120, na=80):
    ...
    for gi in g:
        row = sum(
            np.sin(ai) ** 2
            * float(F(jnp.asarray(nu / (1.5 * gi**2 * np.sin(ai)))))
            * da
            for ai in a
        )
        tot += gi**-p * row * gi * dl
...
def test_curvature():
    ...
    w = np.exp(-0.5 * (ps - pbar) ** 2 / sigp**2)
    w /= w.sum()
    nus = np.logspace(0, 2, 10)
    lnj = np.array(
        [
            np.log(np.sum(w * np.array([power_law_emissivity_rel(nu, p) for p in ps])))
            for nu in nus
        ]
    )
...
def test_abs_emissivity(p=2.5, nu=1e9, C=1.0, B=1e-6):
    ...
    pref = np.sqrt(3) * E_ESU**3 * C * B / (8 * np.pi * M_E * C_CGS**2)
...
def test_running_index(p0=2.5, a=0.02):
    """Formal extended-range log-parabola: running a/2 plus O(a^2).

    p_func is the exponent p0+a*ln(gamma); the local slope is p0+2a*ln(gamma).
    ...
    alpha = running_spectral_index(
        nus, lambda g: p0 + a * np.log(g), gmin=1e-5, gmax=1e5, ng=1200
    )
...
if __name__ == "__main__":
    ...
    print(
        f"  slope={slope:.4f}  a/2={a2:.4f}   a_s(nu=1)={a_mid:.4f} vs pred {p_mid:.4f}"
    )
