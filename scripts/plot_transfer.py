...
(a) Kirchhoff closure -- the absorbed spectrum built from the emissivity
    ...
    over the full N(gamma).  This UR, vanishing-endpoint example supplies the inverse moment independently.
(b) Polarisation through the self-absorption turnover for power-law electron
    spectra: the signed thin fraction -(p+1)/(p+7/3) flips sign to the self-absorbed
...
(c) Magnus expansion of the path-ordered LOS transfer: Omega1 (LOS-averaged
    ...
    deterministic ordered commutator), against the numerical slab chain.
...
def panel_polarisation(ax, nu_c_ref=1.0):
    ...
    for c, p in zip(["C0", "C1", "C2"], [2.0, 2.5, 3.0]):
        N = g**-p
        jF = emissivity(N, g, 100.0, nu_c_ref)
        jG = emissivity_Q(N, g, 100.0, nu_c_ref)
        aF = absorption(N, g, 100.0, nu_c_ref)
        aG = absorption_Q(N, g, 100.0, nu_c_ref)
        S_parallel = (jF + jG) / (2 * (aF + aG))
        S_perpendicular = (jF - jG) / (2 * (aF - aG))
        t_parallel = taus * (aF + aG) / aF
        t_perpendicular = taus * (aF - aG) / aF
        parallel = S_parallel * (-np.expm1(-t_parallel))
        perpendicular = S_perpendicular * (-np.expm1(-t_perpendicular))
        I = parallel + perpendicular
        Q = parallel - perpendicular
    ...
    ax.text(
        0.04,
        0.30,
        r"$\cdots\;\Pi_{\rm thin}=-\frac{p+1}{p+7/3}$",
        transform=ax.transAxes,
        fontsize=8.5,
    )
    ax.text(
