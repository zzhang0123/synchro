"""Provenance records for ``synchro.model`` (private helper of ``errors.py``).

Split out of ``synchro/model/errors.py`` to keep that file under 400 lines;
import these names from ``synchro.model.errors``. Both classes are frozen
...
def strict_json(value):
    ...
    ``synchro.model`` so the output survives a JSON round trip unchanged.
    """
