...
Private helper of :mod:`synchro.model.assumptions`, split out to keep that
...
def affine_pieces(pm, reference):
    ...
    needs_reference = any(
        c.kind not in ("uniform_mu", "fixed_table") for c in pm.closures
    )
...
def record_kinds(pm):
    """``closure_kind`` of the record: kinds joined by ``+``, fitted ones suffixed."""
    kinds = "+".join(
        c.kind + (FITTED_SUFFIX if i in pm.free_hyper else "")
...
def prepare(index, groups, closures, free_hyper, nodes, discrepancy, ErrorTerm):
    ...
    Returns ``(tables_spec, gathers, keep, ext_ok)``; ``ValueError`` on any
    invalid closure, hyper selection, nodal configuration, discrepancy shape
    or fixed-table length.
    """
    for closure in closures:
        _f.validate_closure(closure, groups, index)
    if len({c.group for c in closures}) != len(closures):
        raise ValueError("at most one closure per group")
    ...
    for closure in closures:
        if closure.kind == "fixed_table":
            n = len(spec[groups.index(closure.group)])
            if jnp.shape(closure.hyper[0]) != (n,):
                raise ValueError(f"fixed_table on {closure.group} needs shape ({n},)")
    return spec, gathers, keep, ext_ok
