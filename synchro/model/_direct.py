...
``synchro.model.predict``. The population/quadrature error of the discrete
...
def _per_sample(samples, kernel, channels, route, use_sample_depth, depth_ref, batch):
    ...
    return jax.lax.map(one, leaves, batch_size=min(int(batch), samples.size))
...
def direct_channel_average(
    ...
    *,
    ...
    ``predict``); ``batch_size >= 1`` samples per ``lax.map`` step. The
    ``numerical`` term is unbounded (no two-quadrature estimate here).
...
__all__ = ["direct_channel_average", "excluded_tail_term", "truncation_term"]
