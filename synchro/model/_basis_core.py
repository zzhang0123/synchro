"""Jitted core and eager helpers of ``synchro.model.basis`` (private).

Split out of ``basis.py`` to keep that file under 400 lines. ``build_core``
evaluates ``kernel.angular_projection`` at the reference and its nested
``jax.jacfwd`` derivatives in ``(z_gamma, z_B)`` through the total Taylor
degree ``N`` (``eq: channel derivative coefficients``), divides by ``r! s!``
...
``screen_exponent`` term. Units and shapes follow ``synchro.model.basis``;
...
def package_version() -> str:
    """This package's ``synchro.__version__`` (single source, see pyproject.toml).

    Installed-distribution metadata is not used: ``synchro`` on PyPI is an
    unrelated project, so ``importlib.metadata.version("synchro")`` can name
    the wrong package.
    """
...
def _columns(rows, index, tensors, n_ch, dtype, weight_axis):
    ...
    for (r, s, b), items in groups.items():
        ...
        if weight_axis:
            tensor = tensor[b]
        block = tensor[(slice(None), slice(None)) + (0,) * r + (1,) * s]  # (n_ch, n_lk)
...
def build_core(kernel, channels, reference, phase, *, index, quadrature):
    ...
    the ``I`` and ``V`` columns (``eq: angular parity``).
    """
    f = projection_in_z(kernel, channels, reference, phase, index, quadrature)
    tensors = taylor_tensors(f, index.truncation.N)
    I_t, V_t, P_t = zip(*tensors)
    n_ch = channels.n_ch
    real = jnp.result_type(I_t[0], 1.0)
    I_basis = _columns(index.h0, index, I_t, n_ch, real, False)
    V_basis = _columns(index.h0, index, V_t, n_ch, real, False)
    cplx = jnp.result_type(P_t[0], 1j)
    P_basis = _columns(index.h2, index, P_t, n_ch, cplx, True)
...
def refined(kernel, channels, route, factor, full, *, cross=False):
    ...
    also ``n_nodes_F``); it has no second route, so ``cross=True`` gives
...
def response_matrix(index, n_ch, I_basis, V_basis, P_basis):
    """``C`` of shape ``(4 n_ch, n_real)``; layout in ``synchro.model.basis``."""
